// 투기장(PvP) 봇 AI — 배도라지 덕의 봇을 그대로 옮겨 왔다(2026-09-18, 투기장 = 덕의 대전 방식 이식).
// 던전 동료 봇은 bot.ts. 스킬은 표적이 가까울 때 준비되는 대로 쓴다(덕에는 없던 것).
...
// sniperErr: 저격이 조준경 한 방이 된 뒤(v1.9.5) 봇 저격수는 각도 오차·손떨림을 이만큼 곱한다 — 안 그러면 봇 옥냥란이 93% 로 판을 찢고, 혼자 하기가 저격 봇에게 계속 한 방에 죽는 판이 된다
...
export function makePvpBot(seed: number, opts: { swap?: boolean } = {}): PvpBotMemory {
  return {
    rng: makeRng(seed),
 ...
 */
function pickTarget(state: GameState, map: GameMap, me: PlayerState, mem: PvpBotMemory): PlayerState | null {
...
export function pvpBotInput(
  ...
  if (me.hp < me.maxHp * 0.65 && state.globes.length > 0) {
    let bx = 0
    let by = 0
    let bd = Infinity
    for (const m of state.globes) {
  ...
  mem.stuckY = me.y
  mem.lastMyHp = me.hp
  // 스킬: 적이 보이고 가까우면 준비되는 대로 (궁극기는 적이 둘 이상 가깝거나 체력이 절반 아래일 때)
  if (enemy && los && me.alive && !me.downed) {
    const near = state.players.filter((e) => isEnemy(me, e) && e.alive && !e.left && len(e.x - me.x, e.y - me.y) < 320).length
    if (me.cd[0] === 0 && dist < 380 && rand(mem.rng) < 0.05) out.buttons |= SKILL_BTNS[0]
    if (me.cd[1] === 0 && dist < 300 && rand(mem.rng) < 0.05) out.buttons |= SKILL_BTNS[1]
    if (me.cd[2] === 0 && (near >= 2 || me.hp < me.maxHp * 0.5) && rand(mem.rng) < 0.08) out.buttons |= SKILL_BTNS[2]
  }
  return out
}

/** 어떤 지점 근처(반경 radius px)의 바닥 타일을 배회 목표로. 못 찾으면 아무 곳 */
function pickWanderNear(map: GameMap, mem: PvpBotMemory, cx: number, cy: number, radius: number): void {
...
function pickWander(map: GameMap, mem: PvpBotMemory): void {
  for (let tries = 0; tries < 20; tries++) {
