...
export class ChatBox {
  ...
  constructor(
    ...
    this.root.innerHTML = `<div class="chat-log"></div><input class="chat-in" type="text" maxlength="${CHAT_MAX}" placeholder="할 말 (Enter 보내기 · Esc 닫기)" autocomplete="off" spellcheck="false" hidden>`
    ...
    this.fadeTimer = window.setInterval(() => this.fade(), 500)
  }
  ...
  add(name: string, text: string, kind: ChatLine['kind']): void {
    const line: ChatLine = { name, text: cleanChat(text), kind, at: performance.now() }
    if (!line.text) return
