...
게임에 넣을 때 고친 것: 면 · 동작 키프레임 · 텍스처를 줄이고(`tools/pack-monsters.py` · 면 줄이기 `tools/simplify.py`), 동작을 모양 키 프레임으로 구워 색을 입혔습니다. 포격 악마는 심연의 군주 모델(balrog demon rig)을 작게 · 다른 색으로 씁니다.
